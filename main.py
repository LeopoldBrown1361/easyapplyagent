...
import re
import pyautogui
import modules.scaffolding.oracle as oracle
...
def state_is_acceptable(time_post_text: str) -> bool:
    if time_post_text == "Unknown":
        return False
    remote_split = time_post_text.split(" ")
    state_split = remote_split[0].split(", ")
    if remote_split[0]== 'Remote':
        print(remote_split[0])
        return True
    else:
        state = state_split[1]
        print(state)
    if len(unacceptable_states) == 0:
        if state in acceptable_states:
            ...
            print(f"State {state} is not in the acceptable states list!")
            return False
    elif len(unacceptable_states) > 0:
        if state not in unacceptable_states:
            return True
        else:
            print(f"State {state} is in the unacceptable states list!")
            return False
    return True
...
def get_job_description() -> tuple[
    ...
    try:
        ...
        try:
            ...
            time_posted_text = "Unknown"
        if not state_is_acceptable(time_posted_text):
            return jobDescription, experience_required, True, skipReason, skipMessage
    except Exception as e:
    ...
    apply = oracle.skip_bool(jobDescription)
    if not apply:
        return jobDescription, experience_required, True, skipReason, skipMessage
    
    else:
        return jobDescription, experience_required, False, skipReason, skipMessage
        ...
        if text: 
            ...
            if not prev_answer or overwrite_previous_answers:
                ...
                if 'experience' in label or 'years' in label: 
                    print('<<<<LABEL>>>>',label) #TODO insert your custom answer to the user experience level
                    answer = oracle.ai_get_exp(label)
                    print("<<ANSWER>>>>>",answer)
                    time.sleep(1)
